...
DESIRED_DISK_OPERATION=${5?Error: no desired disk operation given, please select -move or -copy}

python label_cases.py "$TCGA_CASES_DIRECTORY_PATH" "$MUTATION_FILE_PATH" "$MUTATION_OF_INTEREST" "$OUTPUT_DIRECTORY_PATH" "$DESIRED_DISK_OPERATION"
